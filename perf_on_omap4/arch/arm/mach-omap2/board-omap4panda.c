...
/* Display LCD */
#define PANDA_LCD_DISPLAY_GPIO 50
static int omap4_panda_enable_lcd(struct omap_dss_device *dssdev)
{
        printk("Now enable Panda_EXP LCD\n");
        gpio_request(PANDA_LCD_DISPLAY_GPIO,"lcd_enable_gpio");
        gpio_direction_output(PANDA_LCD_DISPLAY_GPIO, 0 );
        gpio_set_value(PANDA_LCD_DISPLAY_GPIO, 1);
...
{
        gpio_set_value(PANDA_LCD_DISPLAY_GPIO, 0);
}
...
struct omap_dss_device omap4_panda_lcd_device = {
        ...
        .data = &lcd_panel,
        .phy.dpi.data_lines = 24,
        .reset_gpio = PANDA_LCD_DISPLAY_GPIO,
        .channel = OMAP_DSS_CHANNEL_LCD2,
};
